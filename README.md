...
5. **Writes** are optimistic and local-first: they are applied to the local db immediately and queued (persisted, ordered across collections so offline-created relations replay parent-first), and the returned promise resolves right away with the optimistic record — it never blocks on the network. The server round trip happens in the background (immediately when online, or on reconnect when offline), and the authoritative server record is merged into the local db when it lands, so live reads and subscriptions update again on confirmation. Server-rejected writes (validation, auth) are rolled back locally and reported via `onSyncError`.
